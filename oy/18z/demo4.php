    ...
    <div class="row">
        <div class="col-xs-12">
            ...
            <form action="" name="myform" class="form form-horizontal" method="post">
                ...
                <?php
                    $link=mysqli_connect('localhost','root','') or die("error").mysql_error();
                    $db_selected=mysql_select_db('db_study',$link);
                    mysqli_query('set names utf8');  // 将数据库编码格式设置为utf8类型，屏蔽乱码

                    $sql=mysqli_query("select * from tb_book");  // 执行查询语句
                    $info=mysql_fetch_array($sql);      // 获取查询结果，返回值为数组

                    if($_POST[Submit]=='提交'){       // 如果按钮为查询
                        $txt_book=$_POST[txt_book];     // 获取文本框提交的值
                        $sql=mysqli_query("select * from tb_book where bookname like '%".trim($txt_book)."%'");
